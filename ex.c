...
int main(void) {
	...
	const float canv_bot_y = SHEIGHT-MARGIN;
	    ...
	    if(!bids.full) {
		    ...
		    DrawTextEx(GetFontDefault(),con,(Vector2){cur_x,pos_t},10,1,WHITE);
		    ...
		    for(int i = 0; i < BUFF_LEN - 1; i++) {
			    ...
			    n = (it+1) % BUFF_LEN;
			    Vector2 snd = {cur_x,canv_bot_y - (bids.vals[n] - min)*y_delta};
			    Vector2 snd2 = {cur_x,canv_bot_y - (asks.vals[n] - min)*y_delta};
			    cur_x -= x_delta;
			    Vector2 fst = {cur_x,canv_bot_y - (bids.vals[it] - min)*y_delta};
			    Vector2 fst2 = {cur_x,canv_bot_y - (asks.vals[it] - min)*y_delta};
        ...
        info.protocols = (struct lws_protocols[]){
            {"binance-ws", callback_binance, 0, 0},
            { NULL, NULL, 0, 0 }
        };
